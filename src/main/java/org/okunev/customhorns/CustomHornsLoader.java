...
public class CustomHornsLoader implements PluginLoader {
    ...
    public void classloader(@NotNull PluginClasspathBuilder classpathBuilder) {
        System.setProperty("customhorns.loader.success", "false");

        Gson gson = new Gson();
        try (InputStream is = getClass().getResourceAsStream(RESOURCE_NAME)) {
            if (is == null)
                throw new FileNotFoundException("Resource not found: " + RESOURCE_NAME);
            ...
            try (InputStreamReader isr = new InputStreamReader(is, StandardCharsets.UTF_8)) {
                ...
                if (data == null)
                    throw new RuntimeException(RESOURCE_NAME + " is empty");
                ...
                if (repositories == null || dependencies == null) {
                    throw new RuntimeException("Missing 'repositories' or 'dependencies' section in " + RESOURCE_NAME);
                }

                MavenLibraryResolver resolver = new MavenLibraryResolver();
                repositories.forEach(url -> {
                    String finalURL = url.equals(MAVEN_CENTRAL) ? getDefaultMavenCentralMirror() : url;
                    String repoID = "repo-" + Math.abs(finalURL.hashCode());
                ...
                System.setProperty("customhorns.loader.success", "true");
            }
        } catch (IOException e) {
            throw new RuntimeException("Failed to process " + RESOURCE_NAME, e);
        }
    }
}